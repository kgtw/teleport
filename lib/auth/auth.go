...
func (s *AuthServer) generateUserCert(req certRequest) (*certs, error) {
	...
	if req.kubernetesCluster, err = CheckKubeCluster(req.kubernetesCluster, s.Presence); err != nil {
		return nil, trace.Wrap(err)
	}
...
// clusters. If kc is empty, this validation succeeds (no error returned).
func CheckKubeCluster(kc string, pg services.ProxyGetter) (string, error) {
	if kc == "" {
		return "", nil
	}
